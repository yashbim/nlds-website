...
// Removed saveMerchPackToDatabase function since it's not needed
...
async function saveOrderToDatabase(customerData: CustomerData) {
  try {
    ...
    const orderResult = await db
      ...
      } as any) // Type assertion to bypass strict typing for auto-generated id field
      .returning('id')
    ...
    await db
      ...
      .values(orderItems as any) // Type assertion to handle potential auto-generated fields
      .execute();

    console.log('Order items saved successfully');

    // Removed the merch pack saving loop since we're not using the merch_packs table
    // All merch pack information is still preserved in the order_items table with:
    // - is_merch_pack: boolean flag
    // - tshirt_size: string
